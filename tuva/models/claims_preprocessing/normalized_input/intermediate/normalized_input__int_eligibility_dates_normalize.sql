...
left join {{ source('terminology', 'calendar') }} cal_dob
    on elig.birth_date = cal_dob.full_date
left join {{ source('terminology', 'calendar') }} cal_death
    on elig.death_date = cal_death.full_date
left join {{ source('terminology', 'calendar') }} cal_enroll_start
    on elig.enrollment_start_date = cal_enroll_start.full_date
left join {{ source('terminology', 'calendar') }} cal_enroll_end
