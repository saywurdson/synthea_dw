...
left join {{ source('terminology', 'calendar') }} cal_claim_start
    on med.claim_start_date = cal_claim_start.full_date
left join {{ source('terminology', 'calendar') }} cal_claim_end
    on med.claim_end_Date = cal_claim_end.full_date
left join {{ source('terminology', 'calendar') }} cal_claim_line_start
    on med.claim_line_start_date = cal_claim_line_start.full_date
left join {{ source('terminology', 'calendar') }} cal_claim_line_end
    on med.claim_line_end_date = cal_claim_line_end.full_date
left join {{ source('terminology', 'calendar') }} cal_admission
    on med.admission_date = cal_admission.full_date
left join {{ source('terminology', 'calendar') }} cal_discharge
